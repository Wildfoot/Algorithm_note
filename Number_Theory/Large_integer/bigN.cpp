//This is brute forces O(N^2)

//fork from http://sunmoon-template.blogspot.tw/2015/01/big-interger.html
...
struct bigN:vector<long long int>{
	const static int base=1000000000,width=log10(base);
	bool negative;
	bigN(const_iterator a,const_iterator b):vector<long long int>(a,b){}
	bigN(string s){
		if(s.empty())return;
	...
	bigN operator/(const bigN &b)const{
		...
		for(int i=int(x.size())-1;i>=0;--i){
			...
			int d=(long long int(base)*s1+s2)/y.back();
			r=r-y*d;
